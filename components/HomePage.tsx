...
export function HomePage(){const [selected,setSelected]=useState<Plan>(plans[3]);const [open,setOpen]=useState(false);const [trialOpen,setTrialOpen]=useState(false);const show=useCallback((p:Plan=plans[3])=>{setSelected(p);setOpen(true)},[]);const close=useCallback(()=>setOpen(false),[]);const showTrial=useCallback(()=>setTrialOpen(true),[]);const closeTrial=useCallback(()=>setTrialOpen(false),[]);return <><Navbar onOrder={()=>show()} onTrial={showTrial}/><main><Hero onOrder={()=>show()} onTrial={showTrial}/><Pricing onOrder={show}/><Entertainment/><TrialCTA onTrial={showTrial}/><Features/><Devices/><HowItWorks/><FAQ onTrial={showTrial}/><Contact/></main><Footer/><OrderModal plan={selected} open={open} onClose={close}/><TrialModal open={trialOpen} onClose={closeTrial}/><Chatbot onTrial={showTrial}/></>}
